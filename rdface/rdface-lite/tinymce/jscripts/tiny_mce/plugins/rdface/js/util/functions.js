...
function show_entities(editor,c){
	if(c=='none'){
		$.each($(editor.getDoc()).find('.r_entity'), function(index, value) { 
			...
			if($(this).hasClass('r_place'))
				$(this).removeClass('r_place').addClass('r_place_h');
			if($(this).hasClass('r_person'))
				$(this).removeClass('r_person').addClass('r_person_h');
			if($(this).hasClass('r_organization'))
				$(this).removeClass('r_organization').addClass('r_organization_h');			
		});		
	}else if(c=='all'){
		$.each($(editor.getDoc()).find('.r_entity_h'), function(index, value) { 
			$(this).removeClass('r_entity_h').addClass('r_entity');
			if($(this).hasClass('r_place_h'))
				$(this).removeClass('r_place_h').addClass('r_place');
			if($(this).hasClass('r_person_h'))
				$(this).removeClass('r_person_h').addClass('r_person');
			if($(this).hasClass('r_organization_h'))
				$(this).removeClass('r_organization_h').addClass('r_organization');			
		});		
	}else{
...
function remove_annotations(editor,only_automatic){
	...
	if(aF=="RDFa"){
		if(only_automatic){
			$(editor.getDoc()).find('.automatic').each(function(){
			...
			$(editor.getDoc()).find('.r_entity').each(function(){
				tmp=$(this).find("span[property='rnews:name']").html();
				if(tmp){
					$(this).replaceWith(tmp);
					$(this).css("background-color","");
		...
		if(only_automatic){
			$(editor.getDoc()).find('.automatic').each(function(){
				tmp=$(this).find("span[itemprop='name']").html();
				if(tmp){
					$(this).replaceWith(tmp);
					$(this).css("background-color","");
				}
...
//Insert RDFa attributes to HTML DOM tree
function insert_entity(editor,entity_type,b){
	...
	if(getCookie("annotationF")=="RDFa"){
		if ((selectedContent == nodeContent) || (selectedContentText==$(selectedContent).html())) {
			editor.dom.setAttrib(selectedNode,"typeof",'rnews:'+entity_type);
			if(uri){
				editor.dom.setAttrib(selectedNode,"about",uri);
			}
			editor.dom.setAttrib(selectedNode,"class",'r_entity r_'+entity_type.toLowerCase());
			selectedNode.innerHTML="<span property='rnews:name'>"+nodeContent+"</span>";
		} else {
			...
			var temp = ' class="r_entity r_'+entity_type.toLowerCase()+'" typeof="rnews:'+entity_type+'"';
			if(uri){
				temp=temp+' about="'+uri+'"';
			}
			var annotatedContent = "<span" + temp + "><span property='rnews:name'>" + selectedContent
			+ "</span></span>";
			if ((editor.selection.getRng().startContainer.data != editor.selection
				...
				annotatedContent = "<div" + temp + "><span property='rnews:name'>" + selectedContent
				+ "</span></div>";
			}
		...
		if ((selectedContent == nodeContent) || (selectedContentText==$(selectedContent).html())) {
			editor.dom.setAttrib(selectedNode,"itemtype",mapTypeToSchema(entity_type));
			if(uri){
				editor.dom.setAttrib(selectedNode,"itemid",uri);
			}
			editor.dom.setAttrib(selectedNode,"class",'r_entity r_'+entity_type.toLowerCase());
			selectedNode.innerHTML="<span itemprop='name'>"+nodeContent+"</span>";
		} else {
			// we need to add a new neutral html tag which involves
			// our annotation attributes
			// to do this we also need to check whether there is a paragraph or
			// not (to use DIV or SPAN)
			var temp = ' class="r_entity r_'+entity_type.toLowerCase()+'" itemscope itemtype="'+mapTypeToSchema(entity_type)+'"';
			if(uri){
				temp=temp+' itemid="'+uri+'"';
			}
			var annotatedContent = "<span" + temp + "><span itemprop='name'>" + selectedContent
			+ "</span></span>";
			if ((editor.selection.getRng().startContainer.data != editor.selection
					.getRng().endContainer.data)
					|| editor.selection.getRng().commonAncestorContainer.nodeName == "BODY") {
				annotatedContent = "<div" + temp + "><span itemprop='name'>" + selectedContent
				+ "</span></div>";
			}
			// editor.selection.setContent(annotatedContent);
			editor.execCommand('mceInsertRawHTML', false,annotatedContent);
...
function showTooltips(editor,b){
	...
	$.each($(editor.getDoc()).find('.r_entity'), function(index, value) { 
		...
		$(this).mouseover(function(e) {								
			...
			$(this).append(' <span id="typeof'+index+'" class="tooltip"><img id="arrow'+index+'" class="arrow"/>'+prepareTooltipContent($(this))+'</span>');
...
function prepareTooltipContent(entityObj){
	if(getCookie("annotationF")=="RDFa"){	
		var entityType=entityObj.attr('typeof').split(':')[1];
		var output="<ul style='list-style: none;text-align:left;margin:0 0 0 0;padding-left: 1em;text-indent: -1em;'><li>Type: <b>"+entityType+"</b></li>";
		switch (entityType){
		case "Place":
			var locType='';
			if(entityObj.find("span[property='rnews:point']").length){
				locType="point";
			}
			if(entityObj.find("span[property='rnews:line']").length){
				locType="line";
			}
			if(entityObj.find("span[property='rnews:polygon']").length){
				locType="polygon";
			}
			if(entityObj.find("span[property='rnews:box']").length){
				locType="box";
			}
			if(entityObj.find("span[property='rnews:circle']").length){
				locType="cirlce";
			}
			if(locType){
				output +="<li>Coordinates: <b>"+locType+"("+entityObj.find("span[property='rnews:"+locType+"']").attr('content')+")</b></li>";
			}
			if(entityObj.find("span[property='rnews:elevation']").attr('content')){
				output +="<li>Elevation: <b>"+entityObj.find("span[property='rnews:elevation']").attr('content')+"</b></li>";
			}
			if(entityObj.find("span[property='rnews:featureCode']").attr('content')){
				output +="<li>Feature Code: <b>"+entityObj.find("span[property='rnews:featureCode']").attr('content')+"</b></li>";
			}		
			break;
		case "Organization":
			if(entityObj.find("span[property='rnews:tickerSymbol']").attr('content')){
				output +="<li>Ticker Symbol: <b>"+entityObj.find("span[property='rnews:tickerSymbol']").attr('content')+"</b></li>";
			}
			break;
		case "Person":
			if(entityObj.find("span[property='rnews:honorificPrefix']").attr('content')){
				output +="<li>Honorific Prefix: <b>"+entityObj.find("span[property='rnews:honorificPrefix']").attr('content')+"</b></li>";
			}
			if(entityObj.find("span[property='rnews:givenName']").attr('content')){
				output +="<li>Given Name: <b>"+entityObj.find("span[property='rnews:givenName']").attr('content')+"</b></li>";
			}
			if(entityObj.find("span[property='rnews:additionalName']").attr('content')){
				output +="<li>Middle Name: <b>"+entityObj.find("span[property='rnews:additionalName']").attr('content')+"</b></li>";
			}
			if(entityObj.find("span[property='rnews:familyName']").attr('content')){
				output +="<li>Family Name: <b>"+entityObj.find("span[property='rnews:familyName']").attr('content')+"</b></li>";
			}
			if(entityObj.find("span[property='rnews:honorificSuffix']").attr('content')){
				output +="<li>Honorific Suffix: <b>"+entityObj.find("span[property='rnews:honorificSuffix']").attr('content')+"</b></li>";
			}
			break;
		}
		output +="</ul>";
	}else{
		var entityType=mapToVocabulary(entityObj.attr('itemtype')).split(':')[1];
		var output="<ul style='list-style: none;text-align:left;margin:0 0 0 0;padding-left: 1em;text-indent: -1em;'><li>Type: <b>"+entityType+"</b></li>";
		switch (entityType){
		case "Place":
			var locType='';
			if(entityObj.find("meta[itemprop='latitude']").length && entityObj.find("meta[itemprop='longitude']").length){
				locType="point";
				output +="<li>Coordinates: <b>"+locType+"("+entityObj.find("meta[itemprop='latitude']").attr('content')+" "+entityObj.find("meta[itemprop='longitude']").attr('content')+")</b></li>";
			
			}else{
				if(entityObj.find("meta[itemprop='line']").length){
					locType="line";
					output +="<li>Coordinates: <b>"+locType+"("+entityObj.find("meta[itemprop='line']").attr('content')+")</b></li>";
				}
				if(entityObj.find("meta[itemprop='polygon']").length){
					locType="polygon";
					output +="<li>Coordinates: <b>"+locType+"("+entityObj.find("meta[itemprop='polygon']").attr('content')+")</b></li>";
				}
				if(entityObj.find("meta[itemprop='box']").length){
					locType="box";
					output +="<li>Coordinates: <b>"+locType+"("+entityObj.find("meta[itemprop='box']").attr('content')+")</b></li>";
				}
				if(entityObj.find("meta[itemprop='circle']").length){
					locType="cirlce";
					output +="<li>Coordinates: <b>"+locType+"("+entityObj.find("meta[itemprop='circle']").attr('content')+")</b></li>";
				}			
			}
			if(entityObj.find("meta[itemprop='elevation']").attr('content')){
				output +="<li>Elevation: <b>"+entityObj.find("meta[itemprop='elevation']").attr('content')+"</b></li>";
			}
			if(entityObj.find("meta[itemprop='featureCode']").attr('content')){
				output +="<li>Feature Code: <b>"+entityObj.find("meta[itemprop='featureCode']").attr('content')+"</b></li>";
			}		
			break;
		case "Organization":
			if(entityObj.find("meta[itemprop='tickerSymbol']").attr('content')){
				output +="<li>Ticker Symbol: <b>"+entityObj.find("meta[itemprop='tickerSymbol']").attr('content')+"</b></li>";
			}
			break;
		case "Person":
			if(entityObj.find("meta[itemprop='honorificPrefix']").attr('content')){
				output +="<li>Honorific Prefix: <b>"+entityObj.find("meta[itemprop='honorificPrefix']").attr('content')+"</b></li>";
			}
			if(entityObj.find("meta[itemprop='givenName']").attr('content')){
				output +="<li>Given Name: <b>"+entityObj.find("meta[itemprop='givenName']").attr('content')+"</b></li>";
			}
			if(entityObj.find("meta[itemprop='additionalName']").attr('content')){
				output +="<li>Middle Name: <b>"+entityObj.find("meta[itemprop='additionalName']").attr('content')+"</b></li>";
			}
			if(entityObj.find("meta[itemprop='familyName']").attr('content')){
				output +="<li>Family Name: <b>"+entityObj.find("meta[itemprop='familyName']").attr('content')+"</b></li>";
			}
			if(entityObj.find("meta[itemprop='honorificSuffix']").attr('content')){
				output +="<li>Honorific Suffix: <b>"+entityObj.find("meta[itemprop='honorificSuffix']").attr('content')+"</b></li>";
			}
			break;
		}
		output +="</ul>";	
